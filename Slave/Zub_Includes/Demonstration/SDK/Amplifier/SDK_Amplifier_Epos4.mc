...
///////////////////////////////////////////////////////////////////////////////
{
    ...
    if (axisPolarity==1){	// set axis polarity
		SdoWrite( nodeid, EPOS4_AXIS_CONFIGURATION, 4, 1);
		print("...: change axis direction");
    }
    else{
		SdoWrite( nodeid, EPOS4_AXIS_CONFIGURATION, 4, 1);
    ...
    // reset Transmittype
    SdoWrite(nodeid, EPOS4_TRANSMIT_PDO_1_PARAMETER, 2, 255);    // TxPDO1 Transmittype
    SdoWrite(nodeid, EPOS4_TRANSMIT_PDO_2_PARAMETER, 2, 255);    // TxPDO2 Transmittype
    SdoWrite(nodeid, EPOS4_TRANSMIT_PDO_3_PARAMETER, 2, 255);    // TxPDO3 Transmittype
    SdoWrite(nodeid, EPOS4_TRANSMIT_PDO_4_PARAMETER, 2, 255);    // TxPDO4 Transmittype

    // the pdos have to be disabled for configuring
    last_value = (SdoRead(nodeid, EPOS4_TRANSMIT_PDO_1_PARAMETER, 1));
    SdoWrite(nodeid, EPOS4_TRANSMIT_PDO_1_PARAMETER, 1, (last_value | 0x80000000)); // disable pdo 1
    last_value = (SdoRead(nodeid, EPOS4_TRANSMIT_PDO_2_PARAMETER, 1));
    SdoWrite(nodeid, EPOS4_TRANSMIT_PDO_2_PARAMETER, 1, (last_value | 0x80000000)); // disable pdo 2
    last_value = (SdoRead(nodeid, EPOS4_TRANSMIT_PDO_3_PARAMETER, 1));
    SdoWrite(nodeid, EPOS4_TRANSMIT_PDO_3_PARAMETER, 1, (last_value | 0x80000000)); // disable pdo 3
    last_value = (SdoRead(nodeid, EPOS4_TRANSMIT_PDO_4_PARAMETER, 1));
    SdoWrite(nodeid, EPOS4_TRANSMIT_PDO_4_PARAMETER, 1, (last_value | 0x80000000)); // disable pdo 4

    //the pdos have to be disabled for configuring
    last_value = (SdoRead(nodeid, EPOS4_RECEIVE_PDO_1_PARAMETER, 1));
    SdoWrite(nodeid, EPOS4_RECEIVE_PDO_1_PARAMETER, 1, (last_value | 0x80000000)); // disable pdo 1
    last_value = (SdoRead(nodeid, EPOS4_RECEIVE_PDO_2_PARAMETER, 1));
    SdoWrite(nodeid, EPOS4_RECEIVE_PDO_2_PARAMETER, 1, (last_value | 0x80000000)); // disable pdo 2
    last_value = (SdoRead(nodeid, EPOS4_RECEIVE_PDO_3_PARAMETER, 1));
    SdoWrite(nodeid, EPOS4_RECEIVE_PDO_3_PARAMETER, 1, (last_value | 0x80000000)); // disable pdo 3
    last_value = (SdoRead(nodeid, EPOS4_RECEIVE_PDO_4_PARAMETER, 1));
    SdoWrite(nodeid, EPOS4_RECEIVE_PDO_4_PARAMETER, 1, (last_value | 0x80000000)); // disable pdo 4

    // Now we setup the correct PDO transmission for Tx and Rx
    SdoWrite(nodeid, (EPOS4_TRANSMIT_PDO_1_PARAMETER + pdonumber-1), 2,  1);			// TxPDO Transmittype SYNC
    SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_PARAMETER + pdonumber-1), 2,  1);				// RxPDO Transmittype SYNC

    // config RX PDO
    SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_MAPPING + pdonumber-1), 0x00, 0);				// disable
    SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_MAPPING + pdonumber-1), 0x01, 0x60400010);	// controlword

    if (operationMode == EPOS4_OP_CSP) {											// cycle synchronous position (csp) mode
    	SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_MAPPING + pdonumber-1), 2, 0x607A0020);	// download pdo 0x1600 entry:	position set point
    	SdoWrite(nodeid, EPOS4_MODES_OF_OPERATION, 0, EPOS4_OP_CSP);					// mode of operation CSP
    	print("...: cycle synchronous position (csp) mode");
    }
    else if (operationMode == EPOS4_OP_CSV ) {
		SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_MAPPING + pdonumber-1), 2, 0x60FF0020);  	// download pdo 0x1600 entry: target velocity 32bit
		SdoWrite(nodeid, EPOS4_MODES_OF_OPERATION, 0, EPOS4_OP_CSV);					// mode of operation CSV
    	print("...: cycle synchronous velocity (csv) mode");
    }
   else if (operationMode == EPOS4_OP_CST) {
		SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_MAPPING + pdonumber-1), 2, 0x60710010);  	// download pdo 0x1600 entry: Target torque 32bit
		SdoWrite(nodeid, EPOS4_MODES_OF_OPERATION, 0, EPOS4_OP_CST);					// mode of operation CST
    	print("...: cycle synchronous torque (cst) mode");
    }
    else {	// Error
		print("...: not supported operation mode");
    ...
    SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_MAPPING + pdonumber - 1), 0x00, 2);     		// enable

    // config TX PDO
    SdoWrite(nodeid, (EPOS4_TRANSMIT_PDO_1_MAPPING + pdonumber - 1), 0x00, 0);     		// disable
    SdoWrite(nodeid, (EPOS4_TRANSMIT_PDO_1_MAPPING + pdonumber - 1), 0x01, 0x60410010);	// statusword
    SdoWrite(nodeid, (EPOS4_TRANSMIT_PDO_1_MAPPING + pdonumber - 1), 0x02, 0x60640020);	// actpos
    SdoWrite(nodeid, (EPOS4_TRANSMIT_PDO_1_MAPPING + pdonumber - 1), 0x03, 0x30D20110);	// Torque actual value averaged
    SdoWrite(nodeid, (EPOS4_TRANSMIT_PDO_1_MAPPING + pdonumber - 1), 0x00, 3 );			// enable

    // fill in the wanted pdo
    SdoWrite(nodeid, (EPOS4_TRANSMIT_PDO_1_PARAMETER + pdonumber - 1), 1, (0x80000180 + (pdonumber - 1)*0x100 + nodeid) );// fill in pdo
    SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_PARAMETER + pdonumber - 1), 1, (0x80000200 + (pdonumber - 1)*0x100 + nodeid) );// fill in pdo

    // enable
    SdoWrite(nodeid, (EPOS4_TRANSMIT_PDO_1_PARAMETER + pdonumber - 1), 1, (0x00000180 + (pdonumber - 1)*0x100 + nodeid) );// enable pdo
    SdoWrite(nodeid, (EPOS4_RECEIVE_PDO_1_PARAMETER + pdonumber - 1), 1, (0x00000200 + (pdonumber - 1)*0x100 + nodeid) );// enable pdo
    ...
    time= Time()%1000;
   	...
   	switch (homingState) {
		...
		case 6:
			...
			return(1);
		default :
			print("...: EPOS4 Homing AxisNo: ",axis," - Incorrect hominState init value: ", homingState);
			return(-1);
	}
